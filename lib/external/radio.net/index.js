...
async function updateStations() {
    const stations = JSON.stringify(await queryLocalStations());
    return new Promise((resolve, reject) => {
        fs.writeFile(`${__dirname}/stations-${Date.now()}.json`, stations, (err) => {
            if (err) return reject(err);
            console.log('The file has been saved!');
        });
        fs.writeFile(`${__dirname}/stations.json`, stations, (err) => {
            if (err) return reject(err);
            console.log('The file has been saved!');
            return resolve();
        });
